...
# 正样本
def cal_and_get_data_res_for_medical(inter = 0.03):
...
def cal_and_get_data_res_for_medical_negative(inter = 0.03,if_all_model = True):
...
def ROC():
    ...
    glove_med_pos = get_similarity_score_from_file(path = 'data/baseline_score/GloVe_pos.txt')
    glove_all_model_neg = get_similarity_score_from_file(path='data/baseline_score/GloVe_neg.txt')
    data_med_pos = cal_and_get_data_res_for_medical()
    data_all_model_neg = cal_and_get_data_res_for_medical_negative(if_all_model= 1)
    #baseline_med_neg = get_similarity_score_from_file(path = 'data/baseline score/med_50000_similarity_score_baseline_negative.txt')
    ...
    while THREASHOLD <= 1:
        ...
        # y_pred_base = baseline_med_pos + random.sample(baseline_all_model_neg, thr)
        ...
        # for comparison
        y_true = [1] * 7666 + [0] * thr
        # y_pred = data_med_pos + random.sample(data_all_model_neg, thr)
        ...
        # for glove
        y_true_glove = [1] * 7666 + [0] * thr
        y_pred_glove = glove_med_pos + random.sample(glove_all_model_neg, thr)
        ...
        ax.plot(fpr_glove,tpr_glove, color = 'green')
...
def get_model_from_txt(path = ''):
    ...
    filename = 'data/syns/medical_syns_7666.txt'
    f = open(filename, 'r')
    count = 0
    total = 0
    res = []
    ...
    for line in f:
        ...
        res += (e1, e2),
        score += word_vectors.similarity(e1,e2),

    for i in res:
        print(i[0]+' '+i[1])
        print(word_vectors.similarity(e1.encode(),e2.encode()))
